# 第11章：失败重试、超时控制与告警基础——把“失败”纳入设计

## 1. 项目背景


在 下游抖动、超时、异常告警联动 这类任务里，团队最容易低估的是“任务系统的运行语义”。开发同学往往把定时任务当作普通方法调用：只要能跑就行；测试同学更关注功能正确性，却忽略重复触发、延迟触发、节点抖动这些分布式变量；运维同学则常在事故发生后才发现任务缺少统一审计、缺少稳定告警、缺少可回放参数。于是看似简单的后台任务，在业务体量扩大后会变成隐蔽风险源。

以 `d:/software/workspace/xxl-job` 仓库为基线，失败治理三件套 这章的核心价值，是把“经验驱动”升级为“机制驱动”。你会看到，`xxl-job-admin` 负责调度编排和治理，`xxl-job-core` 负责执行协议和线程模型，`xxl-job-executor-samples` 用于快速验证链路。当这三层协同起来，团队才真正拥有“可观测、可恢复、可演进”的任务体系。
...
## 3. 项目实战
### 3.1 目标与实施策略
本章实战围绕 `resilienceDemoJob` 展开，目标不是写更多代码，而是完成一个“从配置到执行到复盘”的稳定闭环。建议你先在本地跑通一次正向链路，再故意注入一次失败场景，最后按照复盘模板回填证据。这样做的好处是：你不仅知道成功怎么发生，也知道失败如何被系统接住。
...
# 2) 启动示例执行器
mvn -pl xxl-job-executor-samples/xxl-job-executor-sample-springboot -am spring-boot:run

# 3) 触发一次任务（示例，按实际 accessToken 调整）
...
# 5) 日志巡检（Windows PowerShell）
Get-ChildItem -Recurse d:/software/workspace/xxl-job -Filter "*.log" | Select-Object -First 5
Select-String -Path "d:/software/workspace/xxl-job/**/*.log" -Pattern "trigger|handle|fail|timeout" -SimpleMatch
```
...
**3条结论**
- 失败治理不是补丁动作，而是参数约束、执行语义、告警处置的一体化设计。
- `failRetryCount`、`timeoutSeconds`、阻塞策略必须与业务幂等策略联动配置，否则重试会放大副作用。
- 判定任务稳定必须以证据链为准：Admin 触发记录、Executor 执行日志、`xxl_job_log` 回写三者一致。

**5条落地清单**
- 统一术语：文档和评审只使用 `Admin`、`Executor`、`JobHandler`、`accessToken`、`failRetryCount`、`timeoutSeconds`。
- 统一参数：任务参数采用 `k=v` 逗号分隔（如 `bizDate=2026-04-22,limit=200`），禁止无键裸值。
- 统一命令：本地验证固定四步（启动 Admin、启动 Executor、触发任务、查询 `xxl_job_log`）。
- 统一注释：命令块注释使用“步骤编号 + 动词”（如 `# 步骤1：启动 Admin`）。
- 统一复盘：每次失败至少沉淀“现象、根因、止损、修复、预防”五段信息。

**3条误区**
- 误区一：触发成功就代表业务成功，忽略执行与回写链路校验。
- 误区二：把重试次数调高当成稳定性方案，忽略幂等与限流。
- 误区三：只在事故后补日志，不在任务设计阶段定义结构化字段。
