...
**场景**：鲜果园的开发工位区，老王（PHP 工程师）、小胖和小白围着显示器，屏幕上显示着 "File not found" 的报错。

---

**老王**：（抓头发）我在 Nginx 里配了 `fastcgi_pass 127.0.0.1:9000;`，但一访问 PHP 页面就报 404。PHP-FPM 明明在跑啊！

**小胖**：让我看看你的配置……哦，`root` 设的是 `/var/www/html`，但 `fastcgi_param SCRIPT_FILENAME` 没设。PHP-FPM 不知道要去哪找 PHP 文件。
...
**老王**：我还有一个问题，用户上传头像时，PHP 的 `$_FILES` 数组是空的，但 Nginx access_log 显示请求确实到了。
...
**小胖**：502 Bad Gateway 又是怎么回事？时有时无的。
...
with open('column/chapter08.md', 'w', encoding='utf-8') as f:
    f.write(content)
print('Chapter 8 written, length:', len(content))
