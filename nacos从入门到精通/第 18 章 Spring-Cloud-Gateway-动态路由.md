# 第 18 章 Spring Cloud Gateway 动态路由：配置驱动路由表

> **条目 ID**：`ch-18` · **Slug**：`scg-dynamic-route-nacos` · **级别**：intermediate

## 1. 项目背景（约 500 字）

### 1.1 业务场景（真实或拟真）

网关团队希望路由规则可随活动变更，而不每次发版网关。

### 1.2 痛点放大

路由 JSON 写错会导致入口瞬间全挂；缺少回滚与灰度会把配置变更变成事故。

### 1.3 流程图（示意）

```mermaid
flowchart LR
  GW[SCG] --> R[RouteDefinition]
  R --> Nacos[Nacos Config]
  Ops[运维变更] --> Nacos
```
...
**【第一回合】**  
**小胖**：这章标题好长，Spring Cloud Gateway 动态路由：配置驱动路由表，能不能一句话讲明白？  
**小白**：落地这一步的验收标准是什么？失败时怎么降级？  
**大师**：先把本章主锚点对齐：**nacos-scg-example → DynamicRouteServiceImplByNacos**。先把对象说清，再谈调参。  
**技术映射**：对象模型 / 主锚点 / 证据链。
...
**小胖**：那我啥时候该看源码？  
**小白**：源码阅读从哪里切入最省时间？  
**大师**：从 `nacos/api` 接口面进入，再进实现模块；本章锚点：**nacos-scg-example → DynamicRouteServiceImplByNacos**。  
**技术映射**：接口优先 + 证据优先。

> 剧情提示：上述对话可替换为你们业务域的真实名词；网关团队希望路由规则可随活动变更，而不每次发版网关。...（场景背景可复用于内部培训）。
...
- **仓库路径**：`nacos/`、`nacos-examples/`、`columns/chapter-anchors.md`（`ch-18`）。
...
**目标**：明确本章唯一入口。  
**操作**：打开 `columns/chapter-anchors.md`，检索 **ch-18**，阅读主锚点说明：**nacos-scg-example → DynamicRouteServiceImplByNacos**。  
**期望（文字）**：能一句话说明本章核心对象。  
...
```java
// 示例：监听 Nacos 中 RouteDefinition JSON（节选，见 nacos-scg-example）
configService.addListener(dataId, group, new Listener() {
    @Override
    public void receiveConfigInfo(String configInfo) {
        RouteDefinition def = JSON.parseObject(configInfo, RouteDefinition.class);
