...
> **条目 ID**：`ch-08` · **Slug**：`naming-object-model` · **级别**：basic

## 1. 项目背景（约 500 字）

### 1.1 业务场景（真实或拟真）

线上出现“实例在但流量异常”，需要把 Instance/Service/健康与控制台展示对齐。

### 1.2 痛点放大

把“已注册”当成“可服务”，忽略健康检查与元数据，会导致灰度失败。

### 1.3 流程图（示意）

```mermaid
flowchart TB
  S[Service] --> I1[Instance1]
  S --> I2[Instance2]
  HC[健康检查] --> S
  MD[元数据] --> I1
```
...
**【第一回合】**  
**小胖**：这章标题好长，服务发现的「对象模型」：Instance、Service、Cluster、健康实例，能不能一句话讲明白？  
**小白**：落地这一步的验收标准是什么？失败时怎么降级？  
**大师**：先把本章主锚点对齐：**nacos/api/.../NamingService.java + 控制台**。先把对象说清，再谈调参。  
**技术映射**：对象模型 / 主锚点 / 证据链。
...
**小胖**：那我啥时候该看源码？  
**小白**：源码阅读从哪里切入最省时间？  
**大师**：从 `nacos/api` 接口面进入，再进实现模块；本章锚点：**nacos/api/.../NamingService.java + 控制台**。  
**技术映射**：接口优先 + 证据优先。

> 剧情提示：上述对话可替换为你们业务域的真实名词；线上出现“实例在但流量异常”，需要把 Instance/Service/健康与控...（场景背景可复用于内部培训）。
...
- **仓库路径**：`nacos/`、`nacos-examples/`、`columns/chapter-anchors.md`（`ch-08`）。
...
**目标**：明确本章唯一入口。  
**操作**：打开 `columns/chapter-anchors.md`，检索 **ch-08**，阅读主锚点说明：**nacos/api/.../NamingService.java + 控制台**。  
**期望（文字）**：能一句话说明本章核心对象。  
