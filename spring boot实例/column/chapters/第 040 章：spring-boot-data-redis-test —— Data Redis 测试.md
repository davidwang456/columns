# 第 040 章：spring-boot-data-redis-test —— Data Redis 测试

> 对应模块：`spring-boot-data-redis-test`。本章定位为**测试基础设施与切片实践**：与「Data Redis 测试」配套的主线模块章节共用业务故事，此处侧重 `@…Test`、Mock、Testcontainers 与 CI 中的稳定复现。

---

## 1 项目背景

团队在联调「Data Redis 测试」相关能力时，最怕**本地能通过、CI 偶发失败**：数据库版本漂移、嵌入式中间件与生产不一致、切片上下文漏载 Security/Web 配置。`{slug}` 模块把测试侧自动配置与可选依赖收敛到可重复的组合里。

若没有专用测试模块与约定，测试代码会复制粘贴 `@Import` 与 `MockBean`，维护成本逼近业务代码。结合本仓库 `module/spring-boot-data-redis-test` 的 `build.gradle`，可以把**最小失败用例**固定成团队模板。

```mermaid
flowchart LR
  svc[Service] --> repo[Repository或客户端]
  repo --> mod["spring-boot-data-redis-test"]
  mod --> store[(存储)]
```

**小结：** 没有 `spring-boot` 对该能力的自动装配与属性命名空间时，团队要在**依赖对齐**、**Bean 生命周期**与**运维接口**上重复投入；引入 `spring-boot-data-redis-test` 后，可以把讨论焦点收束到业务约束与 SLA。

---

## 2 项目设计（剧本式交锋对话）

**场景：** 测试架构周会，议题是「如何把 `Data Redis 测试` 测得又快又真」。

**小胖：** 不就写个 `@Test` 吗？为啥还要单独一个模块？跟多打一份外卖有啥区别？

**大师：** 单元测试像「试吃一口」；切片测试像「同一厨房出餐流程」——只启动 MVC 或 JPA 子上下文，速度才上得来。  
**技术映射：** `spring-boot-data-redis-test` 提供测试侧自动配置与 Testcontainers 可选集成。
...
- 构建：Maven 或 Gradle；依赖以官方 **Starter** 或模块文档为准，源码对照 `spring-boot-main/module/spring-boot-data-redis-test/build.gradle`。
...
**步骤 2 — 目标：** 编写与「Data Redis 测试」相关的最小业务或基础设施代码。
...
建议新建示例工程 `demo-data-redis-test`，附 `README` 说明外部依赖（Docker Compose / 本地中间件）。**仓库占位：** `https://example.com/demo/spring-boot-data-redis-test.git`
...
- 对照 `spring-boot-main/module/spring-boot-data-redis-test/README.adoc`（若存在）与 `*Properties`。
...
1. 若要在不修改 `spring-boot-data-redis-test` 自带 `AutoConfiguration` 的情况下替换其中一个 Bean，你会用 `@Bean @Primary`、`@Qualifier` 还是 `BeanDefinitionRegistryPostProcessor`？各自适用什么顺序风险？
2. 与「Data Redis 测试」最相关的生产指标与健康检查项是什么？如何在预发用 Actuator 与 Micrometer 验证？

**参考答案：** 见 [附录：思考题参考答案](../appendix/thinking-answers.md)（可按序号 `040` 维护）。
