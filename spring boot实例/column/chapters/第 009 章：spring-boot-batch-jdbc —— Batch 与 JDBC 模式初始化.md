# 第 009 章：spring-boot-batch-jdbc —— Batch 与 JDBC 模式初始化

> 对应模块：`spring-boot-batch-jdbc`。本章在**基础**层面讲清 `Batch 与 JDBC 模式初始化` 的起步依赖与关键属性；**中级**讨论多环境、容量与可观测性衔接；**高级**指向条件装配、源码中的 `*Properties` 与自定义扩展。
...
`Batch 与 JDBC 模式初始化` 与 Spring Batch / 调度生态结合时，Boot 的自动配置能把 JobRepository、DataSource 与事务边界对齐。

```mermaid
flowchart LR
  boot[SpringBoot应用] --> mod["spring-boot-batch-jdbc"]
  mod --> ext[外部依赖或运行时]
```

**小结：** 没有 `spring-boot` 对该能力的自动装配与属性命名空间时，团队要在**依赖对齐**、**Bean 生命周期**与**运维接口**上重复投入；引入 `spring-boot-batch-jdbc` 后，可以把讨论焦点收束到业务约束与 SLA。

---

## 2 项目设计（剧本式交锋对话）

**场景：** 架构评审室，白板标题为「Batch 与 JDBC 模式初始化」。

**小胖：** 这不就跟夜班流水线一样吗？为啥不能我自己接根线完事？

**大师：** 业务要的是**可复制的正确默认**：连接、超时、序列化、指标，一旦散落各处，线上就是拼图游戏。  
**技术映射：** `spring-boot-batch-jdbc` 把横切关注点收敛到 `*AutoConfiguration` 与 `*Properties`。

**小白：** 哪道工序卡住要亮红灯？如果队头阻塞或下游抖动，我们怎么降级？

**大师：** 先分清**同步还是异步**、**强一致还是最终一致**；再选熔断、隔离、重试与幂等键。Boot 不负责替你选模式，但让集成成本可控。  
**技术映射：** Resilience4j/Micrometer 等与 `Batch 与 JDBC 模式初始化` 常一起出现。
...
**小白：** 和同类技术相比，我们何时不该用 `Batch 与 JDBC 模式初始化`？

**大师：** 当组织已有成熟平台 SDK 且与 Spring 生命周期冲突、或资源极度受限时，要评估**引入成本**与**退出策略**。  
**技术映射：** 技术选型 = **约束优化**，不是功能清单。

**技术映射（小结）：** `Batch 与 JDBC 模式初始化` 对应的自动配置类由 `spring-boot-batch-jdbc` 提供，入口注册见 `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports`（以当前 Boot 版本为准）。
...
- 构建：Maven 或 Gradle；依赖以官方 **Starter** 或模块文档为准，源码对照 `spring-boot-main/module/spring-boot-batch-jdbc/build.gradle`。
...
**步骤 2 — 目标：** 编写与「Batch 与 JDBC 模式初始化」相关的最小业务或基础设施代码。
...
建议新建示例工程 `demo-batch-jdbc`，附 `README` 说明外部依赖（Docker Compose / 本地中间件）。**仓库占位：** `https://example.com/demo/spring-boot-batch-jdbc.git`
...
- 对照 `spring-boot-main/module/spring-boot-batch-jdbc/README.adoc`（若存在）与 `*Properties`。
...
1. 若要在不修改 `spring-boot-batch-jdbc` 自带 `AutoConfiguration` 的情况下替换其中一个 Bean，你会用 `@Bean @Primary`、`@Qualifier` 还是 `BeanDefinitionRegistryPostProcessor`？各自适用什么顺序风险？
2. 与「Batch 与 JDBC 模式初始化」最相关的生产指标与健康检查项是什么？如何在预发用 Actuator 与 Micrometer 验证？

**参考答案：** 见 [附录：思考题参考答案](../appendix/thinking-answers.md)（可按序号 `009` 维护）。
