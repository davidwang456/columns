...
**技术映射**：**Topic** ≈ 逻辑通道名；**Message** ≈ 通道上传输的数据类型（字段固定、可版本化）。
...
**技术映射**：**单话题多 Publisher** 可能带来**顺序与来源语义模糊**；工程上优先 **分话题** 或 **单源**。
...
**技术映射**：**背压**与**队列深度**由 **QoS History/Depth** 等控制；本章先用默认，避免一次塞太多概念。
...
**技术映射**：**ROS 2 CLI** 基于同一套类型支持与 graph 信息；与 **DDS 发现** 一致（中级篇 **M01** 展开）。
...
**技术映射**：**`.msg` 版本策略** ∈ **契约测试**；DDS 只保证「**类型匹配才能建链**」。
