...
**环境**：单测或 `main`；可选安装 **wrk/ab** 或 JMeter；Tomcat 应用可选。

### 步骤

1. **同步 vs 批量**  
   - 写 1000 个 `RBucket` 或 `RBatch`/`pipeline`（按当前 API）写入 `lab:tp:0000`～`lab:tp:0999`。  
   - 用 `System.nanoTime()` 或 Micrometer 记录 **总耗时** 各跑一次（预热 1 轮再计）。  
   - `redis-cli` 抽样 `GET lab:tp:042` 确认写入成功。

2. **（可选）Tomcat 线程**  
   - 提供一个 **同步** Servlet/Controller：循环内 `redisson.getBucket(...).get()` 100 次。  
   - 再提供 **异步** 版本：`getAsync` 链式组合返回 `CompletableFuture`。  
   - 压测 30s，采集 **`http-nio-*` active count** 峰值（Actuator、JMX 或线程 dump）。

3. **可观测性**  
   - 在异步链上 **故意** 抛异常，检查日志是否带 **同一 traceId**（若使用 MDC/Tracing）。若无，补一行 MDC 后再试。

### 验证标准

- 有表格：**同步总耗时 vs 批量总耗时**（同一机器、同一 Redis）。  
- 能回答：**异步是否降低了 Tomcat worker 占用**；异常路径日志 **能否关联一次请求**。

### 记录建议

- 结论写进 Wiki：**「本服务 Redis 调用默认走同步还是异步」** 及理由。
...
**上一章**：[第二章](02-配置-拓扑与调参.md)｜**下一章**：[第四章](04-Codec与序列化.md)
