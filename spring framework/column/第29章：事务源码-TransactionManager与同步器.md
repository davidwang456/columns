# 第 29 章：事务源码——TransactionManager 与同步器
...
在 `commit` / `rollback` 下断点，配合 **第 14 章** 的 `REQUIRES_NEW` 示例，观察 **suspend/resume** 日志。
...
1. **`HandlerMapping`** 与 **`HandlerAdapter`** 分工？（第 30 章。）  
2. **`@RequestBody`** 解析时机？（第 30 章。）
