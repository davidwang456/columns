...
2. **Lazy 事务外**：Open Session In View 关闭后，**懒加载**代理无法初始化 → `LazyInitializationException`（第 23 章用 **@Transactional(readOnly)** 或 **JOIN FETCH** 解决）。
...
**大师**：酷的是**账单**：CPU、代理对象、日志量都会上来。切点表达式能用 **`@annotation` / `within` / `bean()`** 就别全包扫描；**编译期织入 / LTW** 等 **AspectJ 全能力** 另当别论（**第 50 章**）。
...
1. `EntityManager` 与 `Repository` 抽象关系？（第 23 章。）  
2. `@Query` 原生 SQL 与 JPQL 选型？（第 23 章。）
