...
*架构白板正中写着 **Orchestration**，四角被人用便利贴围殴。*
**小胖**：文档里 Delegate **啥都能调**，那把业务全塞进去不就**一个项目搞定**？  
**小白**：蜜月期爽，离婚时哭——引擎管**编排与推进**，不是替你养**领域模型**和全套事务哲学。  
**大师**：想成**交通指挥**：调度员**不踩油门**，但要掌握每辆车在路网哪一段。  
**小白**：同步服务任务 vs External Task，负载和**炸裂姿势**有何不同？微服务各抱一库，一致性讲 **Saga** 还是「最终一致 + 补偿」？报表用 **Runtime API** 扫表行不行？  
**大师**：嵌入式 **同 JVM**：时延低、事务好绑，伸缩跟进程绑死。**External Task**：Worker 独立扩缩、异构语言、对齐 **HPA**，代价是网络、锁、重试设计。一致性？多半是**逐步本地事务 + 流程层补偿**——别指望 2PC 盖全宇宙；错误边界只是表达，规则还是领域里说人话。查询：**运行态走 API**；重型报表**投影到读库/搜索**，别拿生产 OLTP 当数据湖犁。东南西北：**北**向入口与查询、**南**向真实改数据、**西**向 IAM、**东**向观测审计——引擎只占**编排那一格**。  
**小胖**：那我们哪三条最容易自爆？  
**大师**：一、流程当唯一数据库；二、一个 Delegate 打五个下游还吞异常；三、网关上堆业务规则却**没测试**，线上**热改图**当补丁——短期续命，长期玄学。  
