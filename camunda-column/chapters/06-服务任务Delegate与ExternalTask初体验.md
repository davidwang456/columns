...
*代码审查窗口，`@Service` 和 `JavaDelegate` 并排投屏。*
**小胖**：Spring Bean 里写业务不就完了？非要实现 `JavaDelegate`，**多一层implements** 给谁看？  
**小白**：引擎按 BPMN 推进时要**在流程事务里回调**你——`execute(DelegateExecution)` 是契约，失败、重试、作业都挂在上面；你私下 `autowired` 调 Service，事务边界和心理边界一起碎。  
**大师**：同步 Delegate：短、快、同事务；一抛未捕获异常，通常回滚 + 作业失败可按策略重试——**业务异常 vs 技术异常**别搅成一锅粥，第二十单专治。  
**小胖**：那我 `Thread.sleep` 等下游五分钟呢？  
**大师**：**线程葬礼**预定。用异步延续把后续扔作业表，或 External Task / 消息等待，把「卡住」建成显式状态；Kafka 消费者解决的是消息语义，External Task 解决的是**工作项被 Worker 拉取**——别混谈成「都是异步」。  
**小胖**：External Task 和 MQ consumer 本质差在哪？  
**大师**：引擎侧生成的是**可被抢占、可重试的外部作业记录**，拉模式 + topic；MQ 是你自管投递语义。**K8s HPA** 弹 Worker 时，External Task 往往更顺——具体第十四单拆给你听。  
