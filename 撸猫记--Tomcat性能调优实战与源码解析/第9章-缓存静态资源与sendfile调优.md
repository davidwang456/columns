...
> 对应原课纲：`【Tomcat调优】训猫师养成记之强筋健骨`

| 小节 | 时长（分钟） | 详细说明 |
|:---|:---:|:---|
| 调教Cache第一式：静态内容和动态内容的处理？ | 20 | 三层缓存（浏览器/CDN/Tomcat）与内容分层策略 |
| 调教Cache第二式：如何配置最大缓存值？ | 10 | `cacheMaxSize`、`cacheObjectMaxSize` 参数实战 |
| 调教Cache第三式：如何配置缓存的存活时间？ | 10 | TTL、ETag/304、发布失效策略 |
| 调教Cache第四式：如何使用send_file？ | 10 | sendfile 适用边界与压测验证方法 |

### 课程安排建议（50分钟）

- 前 20 分钟讲缓存策略与资源分层
- 后 30 分钟讲参数、TTL 与 sendfile 对比实验
