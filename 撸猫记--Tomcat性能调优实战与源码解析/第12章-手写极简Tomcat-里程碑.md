...
## 课程版扩展（对应 `优化1.md` 第10章）

> 对应原课纲：`综合实战：手写极简Tomcat`

| 小节 | 时长（分钟） | 详细说明 |
|:---|:---:|:---|
| 手写tomcat之实现一个的阻塞HttpServer，支持同步请求响应 | 30 | 完成阻塞IO版本的最小HTTP服务器 |
| 手写tomcat之再增加一个的非阻塞httpServer，支持异步请求响应 | 30 | 升级为Selector/NIO模型，减少线程占用 |
| 手写tomcat之再实现一个的websocket server，支持长连接 | 30 | 完成Upgrade握手与最小帧收发 |
| 手写tomcat之再实现分布式session | 30 | 通过Redis或签名Cookie实现多节点会话保持 |
| 手写tomcat之扩展 | 30 | 增加Filter链/Pipeline雏形/热加载任一扩展 |

### 课程安排建议（150分钟）

- 前 60 分钟：M1 + M2（同步与非阻塞）
- 中 30 分钟：M3（WebSocket）
- 后 60 分钟：M4 + 扩展（分布式会话与工程化）
