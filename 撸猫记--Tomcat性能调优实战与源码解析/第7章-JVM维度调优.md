...
> 对应原课纲：`【Tomcat调优】训猫师养成记之头脑养成`

| 小节 | 时长（分钟） | 详细说明 |
|:---|:---:|:---|
| 调教JVM第一式：JVM性能调优之初识JVM参数 | 10 | JVM参数分类、常见参数作用与误区 |
| 调教JVM第二式：java heap如何调优？ | 20 | Young/Old、GC日志解读、堆大小策略 |
| 调教JVM第三式：tomcat process heap如何调优？ | 30 | Tomcat进程内存画像：堆、线程栈、直接内存联动 |
| 调教JVM第四式：看懂OutOfMemory异常日志 | 30 | OOM类型快速定界、jcmd/jmap/jstat 实战 |

### 课程安排建议（90分钟）

- 10 分钟参数扫盲 + 20 分钟堆与 GC
- 30 分钟进程级调优联动（承接第8章）
- 30 分钟 OOM 排障演练与工具链实操
