...
**小白**：教程让我 `BF.ADD`，我一行敲下去报错……

**大师**：先 `MODULE LIST`。空列表时，别骂教程——先问：**我这颗二进制当初怎么编的？**

**小白**：我 Docker 拉的官方镜像，还会有缺的吗？

**大师**：**会**。镜像说明里写清带了啥；**带 `*` 的概率模块**常常要**自建镜像**或换「全模块」构建。见下文对照表。

**小白**：那我 `make` 两次，目录里会不会乱？

**大师**：用**不同安装前缀**、或**单独 build 目录**，或固定脚本参数。别靠人肉记「昨天到底 make 了啥」。

**小白**：生产也要全模块吗？

**大师**：**最小权限、最小镜像**——只带业务要用的。全模块是**开发机爽**，不是**线上勋章**。
